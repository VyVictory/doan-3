...
export default function SearchBar({ query }) {
    ...
    return (
        <form action="" className="relative w-full max-w-sm justify-center">
            <input
                type="text"
                value={searchTerm}
                onChange={handleInputChange}
                className=" w-72 rounded-xl pr-10 pl-10 py-2 text-black"
                placeholder="Tìm kiếm..."
            />
            {searchTerm && (
                <button onClick={handleClearSearch} className='absolute right-7 top-[40%] transform -translate-y-1/2 text-gray-400 h-4 w-4'>
